...
const RecentMessages = () => {
    ...
    useEffect(()=> {
        if(user){
            fetchRecentMessage();
            setInterval(fetchRecentMessage, 3000)
                    ...
                    <Link to={`/messages/${message.from_user_id._id}`} key={index} className="flex items-start gap-2 py-2 hover:bg-slate-100">
                        ...
                        <div className="w-full">
                            ...
                            <div className="flex justify-between">
                                ...
                                {!message.seen && <p className="flex bg-indigo-500 text-white w-4 h-4 items-center justify-center rounded-full text-[10px]">1</p>}
                            </div>
